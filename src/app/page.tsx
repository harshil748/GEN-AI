  ...
  return (
    <div className="min-h-screen">
      ...
      <header className="sticky top-0 z-30 border-b bg-background/80 backdrop-blur">
        <div className="mx-auto max-w-6xl px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img
              src="https://images.unsplash.com/photo-1531482615713-2afd69097998?q=80&w=200&auto=format&fit=crop"
              alt="MargdarshAI logo"
              className="h-8 w-8 rounded"
            />
            <div>
              <p className="font-semibold leading-tight">MargdarshAI</p>
              <p className="text-xs text-muted-foreground leading-tight">Personalized Career & Skills Advisor</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="ghost" onClick={() => (window.location.href = "/profile")}>Edit Profile</Button>
      ...
      <section className="mx-auto max-w-6xl px-4 py-8">
        <div className="rounded-xl bg-gradient-to-r from-secondary to-accent p-6 border">
          ...
          <p className="text-muted-foreground mt-1">
            Describe your academics, interests, and aspirations. Then start your personalized guidance.
          </p>
        ...
        <Card>
          ...
          <CardContent className="grid gap-4">
            <div>
              ...
              <Input value={subjects} onChange={(e) => setSubjects(e.target.value)} placeholder="Operating Systems, DSA, Networks" />
            </div>
            <div>
              <Label>Extracurricular Interests (comma separated)</Label>
              <Input value={interests} onChange={(e) => setInterests(e.target.value)} placeholder="Cyber security, Cloud, DevOps" />
            </div>
            <div>
              <Label>Skills (comma separated)</Label>
              <Input value={skills} onChange={(e) => setSkills(e.target.value)} placeholder="Linux, Git, Python" />
            </div>
            <div>
              <Label>Vague Aspirations</Label>
              <Textarea value={aspirations} onChange={(e) => setAspirations(e.target.value)} placeholder="I want to work on scalable systems and security..." />
            </div>
            <div className="flex flex-wrap gap-3">
              <Button onClick={startGuidance}>Start Career Guidance</Button>
              <Button variant="outline" onClick={() => setShowBlueprint((v) => !v)}>
                {showBlueprint ? "Hide Skill Blueprint" : "View Skill Blueprint"}
              </Button>
              <Button variant="secondary" onClick={exportPDF}>Generate Career Paths & Export</Button>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="flex flex-col gap-3">
            <div className="min-h-40 max-h-64 overflow-auto rounded border p-3 space-y-2 text-sm">
              {messages.length === 0 && (
                <p className="text-muted-foreground">Say: "I like cloud and cybersecurity"</p>
              )}
              {messages.map((m, i) => (
                  ...
                  <div className="space-y-2">
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground">
                        Demand: {c.market_trends.demand} • Salary: {c.market_trends.salary_range} • Growth: {c.market_trends.growth}
                      </p>
                    </div>
                  </div>
                )}
                <div className="pt-2 flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => setShowBlueprint((v) => !v)}>
                    {showBlueprint ? "Hide Blueprint" : "View Skill Blueprint"}
                  </Button>
                  <Button size="sm" onClick={exportPDF}>Export</Button>
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      <footer className="mx-auto max-w-6xl px-4 py-8 text-xs text-muted-foreground">
        © {new Date().getFullYear()} MargdarshAI • Demo prototype for hackathon. Deterministic outputs for repeatable demos.
      </footer>
    </div>
  );
}